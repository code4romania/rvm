...
class SummaryExpiringProtocols extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail(object $notifiable): MailMessage
    {
        return (new MailMessage)
            ->subject(__('email.summary_expiring_protocols.subject', [
                'count' => $this->organisations->count(),
            ]))
            ->line(__('email.summary_expiring_protocols.subject', [
                'count' => $this->organisations->count(),
            ]))
            ->action(__('email.summary_expiring_protocols.view'), static::actionUrl())
    ...
    public function toDatabase(object $notifiable): array
    {
        return FilamentNotification::make()
            ->title(__('email.summary_expiring_protocols.subject', [
                'count' => $this->organisations->count(),
            ]))
            ->actions([
                Action::make('view')
                    ->label(__('email.summary_expiring_protocols.view'))
